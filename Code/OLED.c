...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	/*如果单片机主频极高(如H7系列)，可在此添加适量延时(如使用__NOP())以避免超出I2C速度*/
	//...
}
...
void OLED_GPIO_Init(void)
{
	...
	/* 2. 将SCL(PB9)和SDA(PB8)引脚初始化为开漏输出模式 */
	GPIO_InitTypeDef GPIO_InitStructure = {0};
	GPIO_InitStructure.Pin = GPIO_PIN_8 | GPIO_PIN_9;
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_OD;       // 开漏输出 (Open Drain)
	GPIO_InitStructure.Pull = GPIO_NOPULL;               // I2C总线通常外部有上拉电阻，设为无上下拉即可
